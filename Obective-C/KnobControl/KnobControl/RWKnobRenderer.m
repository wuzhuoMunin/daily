...
- (void)updateTrackShape{
    ...
    CGFloat offset = MAX(self.pointerLength, self.lineWitdth / 2.f);
...
- (void)updatePointerShape{
    ...
    [pointer moveToPoint:CGPointMake(CGRectGetWidth(self.pointerLayer.bounds) - self.pointerLength -self.lineWitdth / 2.f, CGRectGetHeight(self.pointerLayer.bounds) / 2.f)];
...
- (void)setLineWitdth:(CGFloat)lineWidth{
    if (lineWidth != _lineWitdth) {
        _lineWitdth = lineWidth;
        self.trackLayer.lineWidth = lineWidth;
        self.pointerLayer.lineWidth = _lineWitdth;
        [self updateTrackShape];
        [self updatePointerShape];
    }
}
